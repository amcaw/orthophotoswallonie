<script lang="ts">
	...
	// Fit "solide" sur les bounds de Bruxelles
	function fitToBrussels(opts?: {animate?: boolean}) {
		...
		const dynamicMinZoom = w < 640 ? 9 : w < 1024 ? 9.5 : w < 1300 ? 10 : 10;

		// Update map minZoom dynamically
		map.setMinZoom(dynamicMinZoom);

		map.fitBounds(brusselsBounds, {
			padding,
